...
LORA_ARGS=(
   ...
   #### debug
   --no-offload-train # has issues when disable no offload train
   --no-offload-rollout # has issues when disable no offload rollout, eval = 0
)
##############################
##############################
##############################
...
ROLLOUT_ARGS=(
   ...
   --rm-type math
   --num-rollout 100
   # --num-rollout 2 # only train 2 stesp
   # --rollout-batch-size 32
   --rollout-batch-size 8 # for testing 
   --n-samples-per-prompt 8
   --rollout-max-response-len 1024
   --rollout-temperature 1

   # --global-batch-size 256
   --global-batch-size 64 # for testing
)
...
WANDB_ARGS=(
   ...
   --wandb-project miles-lora-megatron-dev
