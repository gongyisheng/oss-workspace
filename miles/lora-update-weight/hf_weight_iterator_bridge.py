    ...
    def get_hf_weight_chunks(self, megatron_local_weights, weight_type: str | None = None):
        ...
        with megatron_bridge_utils.patch_megatron_model(self.model):
            ...
            named_weights = self._bridge.export_hf_weights(
                ...
                merge_adapter_weights=self.is_lora
            )
            ...
            named_weights_list = list(named_weights)
            for name, tensor in named_weights_list:
                print(f"[bridge export] {name}  shape={tuple(tensor.shape)}  dtype={tensor.dtype}")
