...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def _update_weights_from_flattened_bucket(
        ...
        # DEBUG: verify first tensor was correctly written into model params
        if reconstructed_tensors:
            import hashlib

            def _hash(x):
                x = x.detach().contiguous().view(-1).view(torch.int8)
                return hashlib.sha256(x.cpu().numpy().tobytes()).hexdigest()[:16]

            params_dict = dict(self.model.named_parameters())
            for name, src_tensor in reconstructed_tensors:
                if name in params_dict:
                    src_hash = _hash(src_tensor)
                    dst_hash = _hash(params_dict[name])
                    print(
                        f"[DEBUG weight_sync] param={name} "
                        f"src_hash={src_hash} dst_hash={dst_hash} match={src_hash == dst_hash}",
                        flush=True,
                    )
