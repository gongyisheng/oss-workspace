...
class LoRAMemoryPool:
    ...
    def load_lora_weight_to_buffer(
        ...
        for layer_id in range(self.num_layer):
            ...
            # DEBUG: log CPU tensor hashes before GPU copy (outside CUDA graph, safe)
            import hashlib
            def _hash_cpu(t):
                t = t.detach().contiguous().view(-1).view(torch.int8)
                return hashlib.sha256(t.numpy().tobytes()).hexdigest()[:16]
            for _name, _w in temp_A_buffer.items():
                if isinstance(_w, torch.Tensor):
                    print(f"[DEBUG pool_load] uid={uid} layer={layer_id} module={_name} lora_A shape={list(_w.shape)} hash={_hash_cpu(_w)}", flush=True)
            for _name, _w in temp_B_buffer.items():
                if isinstance(_w, torch.Tensor):
                    print(f"[DEBUG pool_load] uid={uid} layer={layer_id} module={_name} lora_B shape={list(_w.shape)} hash={_hash_cpu(_w)}", flush=True)
