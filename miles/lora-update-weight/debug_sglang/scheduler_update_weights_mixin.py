...
class SchedulerUpdateWeightsMixin:
    ...
    def resume_memory_occupation(
        ...
        if GPU_MEMORY_TYPE_WEIGHTS in tags:
            ...
            torch.distributed.barrier(self.tp_cpu_group)
            # DEBUG: check a param immediately after resume (before import_static_state)
            _debug_log_param(self.tp_worker.model_runner.model, "after_resume_before_import")
            _import_static_state(
                self.tp_worker.model_runner.model,
                self.stashed_model_static_state,
            )
            # DEBUG: check param again after import_static_state
            _debug_log_param(self.tp_worker.model_runner.model, "after_import_static_state")
...
def _compute_hash_tensor(x: torch.Tensor) -> str:
    import hashlib
    x = x.contiguous().view(-1).view(torch.int8)
    return hashlib.sha256(x.cpu().numpy().tobytes()).hexdigest()[:16]


def _debug_log_param(model, tag: str):
    """Log the first named parameter hash and any LoRA output_offset buffers for debugging."""
    params = dict(model.named_parameters())
    for name, param in params.items():
        print(
            f"[DEBUG weight] [{tag}] param={name} hash={_compute_hash_tensor(param.detach())}",
            flush=True,
        )